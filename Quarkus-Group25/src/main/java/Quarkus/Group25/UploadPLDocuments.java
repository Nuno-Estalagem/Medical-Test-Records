...
@RolesAllowed("partnerLab")
@Path("/upload")
public class UploadPLDocuments {
    ...
    public byte[] uploadDocument(String request,@RestHeader("Timestamp") String timestamp) throws Exception {
        ...
        try {
            ...
            PartnerLabDocument plDocumento = new PartnerLabDocument();
            plDocumento.document = document;
            plDocumento.uploader = object.getString("username");
            plDocumento.lab = "partner-lab";
            plDocumento.hash = CipherHandler.hashMessage(plDocumento.toString().getBytes());
            plDocumento.persist();
            patient.foreignDocumentsIds.add(plDocumento.id);
            patient.update();
